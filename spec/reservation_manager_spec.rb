...
describe "Reservation Manager" do
  before do
    @frontdesk = Hotel::ReservationManager.new(5)
  end
  ...
  it "raises an Argument Error if hotel has more than 20 rooms" do
    expect{Hotel::ReservationManager.new(21)}.must_raise(ArgumentError)
  end

  it "displays all available rooms" do
    expect(@frontdesk.total_rooms).must_be_instance_of(Array)
    expect(@frontdesk.total_rooms.first).must_equal(1)
    expect(@frontdesk.total_rooms[3]).must_equal(4)
    expect(@frontdesk.total_rooms.length).must_equal(5)
  end

  describe "book reservation method" do
    before do
      @frontdesk = Hotel::ReservationManager.new(20)

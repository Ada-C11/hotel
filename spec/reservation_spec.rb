...
describe "Reservation class" do
  ...
  describe "Reservation#valid_date_range?" do
    it "check_in is before check_out" do
      expect(valid_reservation.valid_unavailable_dates?(check_in: date1, check_out: date2)).must_equal true
    end

    it "check_in is same as end" do
      expect(valid_reservation.valid_unavailable_dates?(check_in: date2, check_out: date2)).must_equal false
    end

    it "check_in is before today" do
      expect(valid_reservation.valid_unavailable_dates?(check_in: past, check_out: date1)).must_equal false
    end
  end

...
def create_block(num_rooms, arrive_day, depart_day, discount)
  rooms = []
  num_rooms.times do |num|
    room = HotelSystem::Room.new(id: num)
    rooms << room
  end
  block = HotelSystem::Block.new(rooms: rooms, arrive_day: arrive_day, depart_day: depart_day, discount: discount)
  return block
end

...
describe "hotel_block class" do
  before do
    ...
    @block = Hotel::Block.new(@checkin_date, @checkout_date, [1,3,4,5], 150, hotel: @hotel)
  end
  ...
  it "raises an error if at least one of the selected rooms is unavailable" do
    ...
    expect{@block2 = Hotel::Block.new(@checkin_date2, @checkout_date2, [1,3,4], 150, hotel: @hotel)}.must_raise ArgumentError
  end
  ...
  it "raises an error if I try to create another block with a room already in another block" do
    ...
    expect{@block3 = Hotel::Block.new(@checkin_date3, @checkout_date3, [2,3], 150, hotel: @hotel)}.must_raise ArgumentError
  end
...
describe "check_block_availability method" do 
  before do 
    ...
    @block = Hotel::Block.new(@checkin_date, @checkout_date, [1,3,4,5], 150, hotel: @hotel)
    @block2 = Hotel::Block.new(@checkin_date2, @checkout_date2, [6,7,8], 150, hotel: @hotel)
...
describe "reserve room from block method" do
  ...
  it "can add that reservation to the hotel's list of reservations" do
    expect(@block.all_reservations[0]).must_be_kind_of Hotel::Reservation

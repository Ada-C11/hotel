...
class ReservationManager
  ...
  # TO DO have a class for make_reservation and another for make_block_reservation
  ...
  #   elsif @blocked_rooms_array.include?(new_reservation.room) && new_reservation.block_name == "no name"
  #     #don't allow block to be booked unless they are part of that block
  #     #adjust to make sure they book the exact blocked duration
  #     raise ArgumentError, "That room is blocked, choose another room"
  #   elsif new_reservation.block_name != "no name" && (@blocked_reservation_array.first.start_date == new_reservation.start_date && @blocked_reservation_array.first.end_date == new_reservation.end_date)
  #     @reservation_array << new_reservation
  #     # this logic cannot account for multiple blocks
  #   elsif new_reservation.block_name != "no name" && (@blocked_reservation_array.first.start_date != new_reservation.start_date || @blocked_reservation_array.first.end_date != new_reservation.end_date)
  ...
  # use this to make a regular reservation. Use make_block_reservation to reserve a room from a block.
  def make_reservation(start_date: Date.today.to_s, end_date: (Date.today + 1).to_s, room: "0", cost: 200, block_name: "no name")
    new_reservation = Reservation.new(start_date: start_date, end_date: end_date, room: room, cost: cost, block_name: block_name)
    check_res = view_available_rooms(start_date: new_reservation.start_date, end_date: new_reservation.end_date)
    if !check_res.include?(new_reservation.room)
  ...
  # ALSO TO DO - Change logic for knowing what is a block - don't have to rely on cost anymore.
